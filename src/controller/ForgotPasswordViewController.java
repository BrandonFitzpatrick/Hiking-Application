...
public class ForgotPasswordViewController {
    ...
    void changePasswordBtnClicked(ActionEvent event) {
    	...
    	if (accountFound != null) {
    		if(isValidPassword(password, confirmPasswordField.getText(), username, 8)) {
    			accountFound.setPassword(password);
				try {
					FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/LogInView.fxml"));
					Parent root = loader.load();
					Scene scene = new Scene(root);
    			...
    			passwordChangeFailed.setText("Passwords Does Not Meet The Criteria, Or Passwords Do Not Match!");
    		}
    	} else {
    		passwordChangeFailed.setText("Account Not Found!");
    	}
    }
